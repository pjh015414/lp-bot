...
DISCOGS_HEADERS = {"User-Agent": "lp-bot/final"}
...
def pick_artist(release_json: dict) -> str:
    arts = release_json.get("artists") or []
    names = []
    for a in arts:
        n = (a.get("name") or "").strip()
        if n:
            names.append(n.replace(" (2)", "").replace(" (3)", ""))
...
def youtube_link(artist: str, track: str, album: str) -> str | None:
    """
    유튜브 링크 최대 성공률 버전:
    - 곡/아티스트/앨범 조합 + audio/topic/official audio 순으로 재시도
    - 음악 카테고리 제한
    """
    ...
    queries = [
        f'"{artist}" "{track}" "{album}" audio',
        f'"{track}" "{artist}" "{album}" audio',
...
# ================== EMBED HELPERS ==================
def chunk_lines(lines: list[str], max_chars: int = 1000) -> list[str]:
    chunks = []
    cur = ""
    for line in lines:
...
def build_embeds(title: str, artist: str, year, country, cover_url: str | None, track_lines: list[str]) -> list[discord.Embed]:
    footer = "LP Bot • " + ("YouTube 있음" if youtube_enabled() else "YouTube 키 없음")

    base = discord.Embed(title=title, description=artist, color=0x2b2d31)

    info = []
    if year:
        info.append(str(year))
    if country:
        info.append(str(country))
    if info:
    ...
    embeds = []
    chunks = chunk_lines(track_lines, max_chars=1000)
    ...
    for i, chunk in enumerate(chunks):
        ...
        # embed field 제한(25개) 대비
        if field_count >= 24:
            embeds.append(cur)
            cur = discord.Embed(title=title, description=artist, color=0x2b2d31)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if tracks:
            ...
            for i, t in enumerate(tracks, 1):
                yt = youtube_link(artist, t, title)
                if yt:
        ...
        embeds = build_embeds(title, artist, year, country, cover, lines)

        # 선택창 메시지를 "결과"로 교체 + 드롭다운 제거
        await interaction.message.edit(content=None, embed=embeds[0], view=None)

        # 긴 경우 나머지 embed 추가 전송
        if len(embeds) > 1:
            for e in embeds[1:]:
                await interaction.channel.send(embed=e)

        # !lp 쓴 사람 메시지 삭제 (권한 필요: Manage Messages)
        try:
...
async def lp(ctx, *, query: str = None):
    # MissingRequiredArgument 방지
    if not query or not query.strip():
        await ctx.send("사용법: `!lp 앨범/아티스트명`  예) `!lp 검정치마 team baby`")
        return
