...
def youtube_link(artist: str, track: str, album: str) -> str | None:
    """
    유튜브 링크가 안 붙는 문제 최종 해결 버전:
    - 아티스트/곡/앨범 모두 넣어서 검색 정확도 올림
    - official audio 없으면 topic, audio, 일반 검색 순으로 fallback
    - 음악 카테고리로 제한
    """
...
class LPSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for i, t in enumerate(tracks, 1):
            yt = youtube_link(artists, t, title)
            if yt:
        ...
        embeds = build_embeds(title, artists, year, country, cover, lines)

        # 선택창 메시지를 결과로 교체 + 드롭다운 제거
